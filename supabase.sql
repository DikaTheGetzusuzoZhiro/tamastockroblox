...
update public.reviews set admin_reply=left(btrim(admin_reply),500) where admin_reply is not null and char_length(admin_reply)>500;
...
alter table public.reviews drop constraint if exists reviews_admin_reply_check;
...
begin
  ...
  begin alter table public.reviews add constraint reviews_admin_reply_check check (admin_reply is null or char_length(admin_reply) between 1 and 500); exception when duplicate_object then null; end;
end $$;
...
begin
  ...
  -- Satu token hanya boleh memiliki satu sesi chat.
  -- Kalau sesi tersebut sudah closed, kembalikan sesi closed
  -- supaya user tidak bisa membuat chat baru dengan token lama.
  select * into c
  from public.chats
  ...
  if c.id is null then
    ...
    values(trim(p_username),p_category,'open',p_token,now(),now())
    returning * into c;
  elsif c.status='open' then
    update public.chats
    set username=trim(p_username), category=p_category, updated_at=now()
    where id=c.id
    returning * into c;
  end if;
  return c;
